...
void GradientCornerDetection(cv::Mat* img, cv::Mat* prev_img,
                             ...
                             std::vector<cv::Point2d>* res, cv::Mat* img_comp, cv::Mat* img_res,
    ...
    for (int row_cursor = kern_center_row; row_cursor < rows - kern_center_row;
        ...
        for (int col_cursor = kern_center_col;
            ...
            grad_vertical_horizontal->at<float>(row_cursor, col_cursor) =
                row_col_grad;
#if GRAPHIC ==  1
            img_comp->at<uchar>(row_cursor, col_cursor)= prev_img->at<uchar>(row_cursor, col_cursor);
            img_comp->at<uchar>(row_cursor, cols + col_cursor) = img->at<uchar>(row_cursor, col_cursor);
#endif
    ...
    for (int row_cursor = kern_center_row; row_cursor < rows - kern_center_row;
        ...
        for (int col_cursor = kern_center_col;
            ...
            if (k < threshold) {
                ...
                if (img_comp != NULL) {
#if GRAPHIC ==  1
                    cv::circle(*img_comp, cv::Point(col_cursor, row_cursor),1, cv::Scalar(0,0,255));
#endif
...
//                                difference =
//
//                                    prev_img->at<float>(row_prev_cursor - 1,
//                                                        col_prev_cursor - 1)
//                                    - img->at<float>(row_prev_cursor - 1,
//                                                     col_prev_cursor - 1)
//
//                                    + prev_img->at<float>(row_prev_cursor,
//                                                          col_prev_cursor - 1)
//                                    - img->at<float>(row_prev_cursor,
//                                                     col_prev_cursor - 1)
//
//                                    + prev_img->at<float>(
//                                        row_prev_cursor + 1,
...
#if GRAPHIC ==  1
                    cv::arrowedLine(*img_comp, cv::Point(col_cursor, row_cursor),cv::Point(cols + min_similarity_col, min_similarity_row), cv::Scalar(0,0,255));
#endif
...
#if GRAPHIC ==  1
//    cv::arrowedLine(*img_comp, cv::Point(poi_winner_col, poi_winner_row),cv::Point(cols + poi_new_winner_col, poi_new_winner_row), cv::Scalar(0,0,255));
#endif
    cv::imshow("speed table", speed_vector_table);
//	std::this_thread::sleep_for(std::chrono::milliseconds(200));

//    std::cout << pt_interet << " " << speed_vector_max << std::endl;
    ux = (float) speed_vector_max_col;
    uy = (float) speed_vector_max_row;

#if GRAPHIC ==  1
    if(img_res != NULL)
    cv::arrowedLine(*img_res, cv::Point(cols/2, rows/2),cv::Point(cols/2+ speed_vector_max_col, rows/2 + speed_vector_max_row), cv::Scalar(255));
#endif
